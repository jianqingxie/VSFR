...
class Transformer(CaptioningModel):
    ...
    def forward(self, images, seq, image_concepts, *args):
        grid_embed = self.get_pos_embedding(images)
        enc_output, mask_enc = self.encoder(images, image_concepts, pos=grid_embed)
    ...
    def step(self, t, prev_output, visual, image_concepts, seq, mode='teacher_forcing', **kwargs):
        it = None
        if mode == 'teacher_forcing':
            ...
            if t == 0:
                ...
                self.enc_output, self.mask_enc = self.encoder(visual, image_concepts, pos=self.grid_embed)
