...
class ScaledDotProductGeometryAttention(nn.Module):
    ...
    def __init__(self, d_model, d_k, d_v, h, dropout=.1, rpe_config=None, comment=None):
        '''
        :param d_model: Output dimensionality of the model
        :param d_k: Dimensionality of queries and keys
        :param d_v: Dimensionality of values
        ...
        self.rpe_q, self.rpe_k, self.rpe_v = irpe.build_rpe(rpe_config, head_dim=d_k, num_heads=h)
    ...
    def forward(self, queries, keys, values, box_relation_embed_matrix, attention_mask=None, attention_weights=None):
        ...
        att = torch.matmul(q, k.permute(0, 1, 3, 2))  # (b_s, h, nq, nk)
        ...
        if self.rpe_q is not None:
            att += self.rpe_q(k / np.sqrt(self.d_k)).transpose(2, 3)
        ...
        # w_mn = 0 + w_a
        w_mn = torch.softmax(att, -1)  # bs * 8 * r * r
...
class MultiHeadGeometryAttention(Module):
    ...
    def __init__(self, d_model, d_k, d_v, h, dropout=.1, rpe_config=None, identity_map_reordering=False, can_be_stateful=False,
                 attention_module=None, attention_module_kwargs=None, comment=None):
        super(MultiHeadGeometryAttention, self).__init__()
        self.identity_map_reordering = identity_map_reordering

        self.attention = ScaledDotProductGeometryAttention(d_model=d_model, d_k=d_k, d_v=d_v, h=h, rpe_config=rpe_config, comment=comment)
