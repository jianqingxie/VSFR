...
class TextField(RawField):
    ...
    def build_vocab(self, *args, **kwargs):
        ...
        counter = Counter()
        sources = [] # train_dataset.text + val_dataset.text + min_freq
        for arg in args:
        ...
        for data in sources:
            for x in data: # x is a caption sentence
                x = self.preprocess(x)
                try:
                    counter.update(x)
                except TypeError:
...
class ConceptField(RawField):
    def __init__(self, concepts=None, vocab=None, preprocessing=None, postprocessing=None):
        super(ConceptField, self).__init__(preprocessing, postprocessing)
        self.concepts = concepts
        self.vocab = vocab

    def preprocess(self, concepts, device=None):

        image_concepts = []
        image_concepts.append([self.vocab.stoi[token] for token in concepts])
        return torch.Tensor(image_concepts).contiguous()
