...
class DictionaryDataset(Dataset):
    def __init__(self, examples, fields, key_fields, concept_fields):
        if not isinstance(key_fields, (tuple, list)):
            key_fields = (key_fields,)
        if not isinstance(concept_fields, (tuple, list)):
        ...
        concept_fields = {c: fields[c] for c in concept_fields}
        value_fields = {k: fields[k] for k in fields.keys() if k not in key_fields and k not in concept_fields}
        key_examples = []
        key_dict = dict()
        value_examples = []
        concept_examples = []
        ...
        for i, e in enumerate(examples):
            ...
            concept_example = Example.fromdict({c: getattr(e, c) for c in concept_fields})
        ...
        self.concept_dataset = Dataset(concept_examples, concept_fields)
        super(DictionaryDataset, self).__init__(examples, fields)

    def collate_fn(self):
        def collate(batch):
            key_batch, value_batch, concept_batch = list(zip(*batch))
            key_tensors = self.key_dataset.collate_fn()(key_batch)
            value_tensors = self.value_dataset.collate_fn()(value_batch)
            concept_tensors = self.concept_dataset.collate_fn()(concept_batch)
            return key_tensors, value_tensors, concept_tensors
        return collate

    def __getitem__(self, i):
        return self.key_dataset[i], self.value_dataset[i], self.concept_dataset[i]
...
class PairedDataset(Dataset):
    def __init__(self, examples, fields):
        ...
        self.concept_field = self.fields['concept']
    ...
    def concept_set(self):
        concept_list = [e.concept for e in self.examples]
        concept_list = unique(concept_list)
        examples = [Example.fromdict({'concept': i}) for i in concept_list]
        dataset = Dataset(examples, {'concept': self.concept_field})
        return dataset

    def image_dictionary(self, fields=None):
        if not fields:
            fields = self.fields
        dataset = DictionaryDataset(self.examples, fields, key_fields='image', concept_fields='concept')
        return dataset
    ...
    def concept_dictionary(self, fields=None):
        if not fields:
            fields = self.fields
        dataset = DictionaryDataset(self.examples, fields, key_fields='concept')
        return dataset
...
class COCO(PairedDataset):
    def __init__(self, image_field, text_field, concept_field, img_root, ann_root, id_root=None, use_restval=True,
        ...
        with nostdout():
            self.train_examples, self.val_examples, self.test_examples = self.get_samples(roots, ids, concept_field.concepts, image_field)
        examples = self.train_examples + self.val_examples + self.test_examples
        super(COCO, self).__init__(examples, {'image': image_field, 'text': text_field, 'concept': concept_field})
    ...
    @classmethod
    def get_samples(cls, roots, ids_dataset=None, concepts=None, image_field=None):
        train_samples = []
        val_samples = []
        test_samples = []
        if image_field.detections_path is not None:
            f = h5py.File(image_field.detections_path, 'r')
        ...
        for split in ['train', 'val', 'test']:
            ...
            for index in range(len(ids)):
                ...
                if image_field.detections_path is not None:
                    if '%d_grids' % img_id not in f:
                        continue
                filename = coco.loadImgs(img_id)[0]['file_name']

                example = Example.fromdict({'image': os.path.join(img_root, filename), 'text': caption, 'concept': concepts[img_id]})
